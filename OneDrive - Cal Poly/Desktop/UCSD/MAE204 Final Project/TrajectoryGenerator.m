...
% Duration of each segment (s)
    ...
    t2 = 1;    % Move the gripper to the grasp position
    t3 = 1;    % Pause to close the gripper (1)
    t4 = 1;    % Move the gripper back to standoff configuration
    t5 = 1;    % Move the gripper to standoff configuration above the final configuration
    t6 = 1;    % Move the gripper to the final configuration of the object
    t7 = 1;    % Pause to open the gripper (1)
    t8 = 1;    % Move the gripper back to the standoff configuration
...
% Initialize the traj cell array
    traj = cell(totalsteps, 2);  % TODO: see if initializing to (say) zero is any faster
    stepcounter = 1;
